...
export default {
  name: "smp-sale-report",
  data() {
    return {
      startDate: moment()
        .startOf("month")
        .format(),
      startDateFormated: moment()
        .startOf("month")
        .format("DD-MM-YYYY"),
      endDate: moment()
        .endOf("month")
        .format(),
      endDateFormated: moment()
        .endOf("month")
        .format("DD-MM-YYYY"),
      ...
      summary: {},
      selectedUser: "",
      selectedUserCommission: 0,
      selectedUserMonthlyRent: 0,
      reportType: {
      ...
      filterUser: {
        ...
        description: __("Allow you to choose the vendor.")
      },
      field: {
        type: "datetimepicker",
        value: "2021-02-07",
  ...
  methods: {
    ...
    setStartDate(moment) {
      ...
      this.startDateFormated = moment.format("DD-MM-YYYY");
    },
    ...
    async openUserFiltering() {
      try {
         ...
         */
        const result = await new Promise((resolve, reject) => {
          nsHttpClient.get(`/api/nexopos/v4/smp/vendors`).subscribe({
            next: users => {
              this.users = users;
          ...
          next: response => {
            if (response.result.length > 0) {
              for (let i = 0; i < response.result.length; i++) {
                response.result[i].created_at = moment(
                  response.result[i].created_at
                ).format("MM/DD");
              }
            }
            this.selectedUserCommission = response.vendor.commission;
            this.selectedUserMonthlyRent = response.vendor.monthly_rent;
    ...
    setEndDate(moment) {
      ...
      this.endDateFormated = moment.format("DD-MM-YYYY");
    }
  }
};
</script>

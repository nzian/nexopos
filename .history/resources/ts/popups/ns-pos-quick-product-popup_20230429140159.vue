...
if ( field.name === 'vendor_id' ) { field.options = result[2].map( vendor => {
return { label: vendor.vendor_id, value: vendor.id, } }) }
                    });
...
}, { label: __( 'Vendor Name' ), name: 'vendor_id', type: 'select', options: [
], description: __( 'Choose a vendor for the product.' ), show( form ) { return

                                        ...
                                        <table class="table ns-table w-full">
                                            <tbody class="text-primary">
                                                <tr class="">
                                                    ...
                                                    <td class="p-2 border text-right border-success-primary">@{{ summery.total_vendor || 0 }}</td>
                                                </tr>
                                                <tr class="">
                                                    <td width="200" class="font-semibold p-2 border text-left bg-info-secondary border-info-primary text-white">{{ __m( 'Sub Total' ) }}</td>
                                                    <td class="p-2 border text-right border-info-primary">@{{ summery.subtotal | currency }}</td>
                                                </tr>
                                                <tr class="">
                                                    <td width="200" class="font-semibold p-2 border text-left bg-error-secondary border-error-primary text-white">{{ __m( 'Sales Commission' ) }}</td>
                                                    <td class="p-2 border text-right border-error-primary">@{{ summery.sales_commission | currency }}</td>
                                                </tr>
                                                <tr class="">
                                                    <td width="200" class="font-semibold p-2 border text-left bg-error-secondary border-error-primary text-white">{{ __m( 'Monthly rent' ) }}</td>
                                                    <td class="p-2 border text-right border-error-primary">@{{ summery.monthly_rent | currency }}</td>
                                                </tr>
                                                <tr class="">
                                                    <td width="200" class="font-semibold p-2 border text-left bg-success-secondary text-white">{{ __m( 'Total Payable' ) }}</td>
                                                    <td class="p-2 border text-right border-success-primary">@{{ summery.total_payable | currency  }}</td>
                                                </tr>
                            ...
                            <table class="table ns-table w-full">
                                ...
                                <tbody class="text-primary">
                                    <tr v-for="vendor of result" :key="vendor.id">
                                        ...
                                        <td class="p-2 border text-right">@{{ vendor.total_sale | currency }}</td>
                                        <td class="p-2 border text-right">@{{ (vendor.commission*vendor.total_sale)/100 | currency }}</td>
                                        <td class="p-2 border text-right">@{{ vendor.monthly_rent | currency }}</td>
                                        <td class="p-2 border text-right" v-if="((vendor.commission*vendor.total_sale)/100 +vendor.monthly_rent))> 0">@{{(vendor.total_sale - ((vendor.commission*vendor.total_sale)/100 +vendor.monthly_rent)) | currency }}</td>
                                    </tr>
                                </tbody>
                                <tfoot class="text-primary font-semibold">
                                    <tr>
                                        <td class="p-2 border text-primary"></td>
                                        <td class="p-2 border text-right text-primary">@{{ summery.subtotal | currency }}</td>
                                        <td class="p-2 border text-right text-primary">@{{ summery.sales_commission | currency }}</td>
                                        <td class="p-2 border text-right text-primary">@{{ computeTotal( result, 'monthly_rent' ) | currency }}</td>
                                        <td class="p-2 border text-right text-primary">@{{ summery.total_payable  | currency }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>

...
<div class="flex-auto flex flex-col">
    ...
    <div class="flex-auto flex flex-col" id="dashboard-content">
        ...
        <smp-sale-report inline-template v-cloak>
                ...
                <div class="flex -mx-2">
                    ...
                    <div class="px-2">
                        <button @click="openUserFiltering()" class="rounded flex justify-between bg-input-button shadow py-1 items-center text-primary px-2">
                            ...
                            <span class="pl-2">{{ __( 'Filter By Vendor' ) }} : @{{ selectedUser || __( 'All Vendors' ) }}</span>
                        </button>
                    </div>
                </div>
                        ...
                        <div class="my-4 flex justify-between w-full">
                            <div class="text-secondary">
                                <ul>
                                    ...
                                    <li class="pb-1 border-b border-dashed">{{ __( 'Document : Monthly Sale Report' ) }} @{{ selectedUser}} @{{ startDateFormated }} - @{{ endDateFormated}}</li>
                                    <li class="pb-1 border-b border-dashed">{{ sprintf( __( 'By : %s' ), Auth::user()->username ) }}</li>
                                    <li class="pb-1 border-b border-dashed">Shawn's Flea Market    285 Prince Royal Drive   Berea, KY  40403    (859) 985-9499</li>
                                </ul>
                            </div>
                            <div>
                                    ...
                                    <div class="border-b ns-box-body">
                                        <table class="table ns-table w-full">
                                            <tbody class="text-primary">
                                                ...
                                                <tr class="">
                                                    <td width="200" class="font-semibold p-2 border text-left bg-error-secondary text-white">{{ __( 'Commission' ) }}</td>
                                                    <td class="p-2 border text-right border-success-primary">@{{ summary.sales_commission | currency }}</td>
                                                </tr>
                                                <tr class="">
                            ...
                            <table class="table ns-table w-full">
                                <thead class="text-primary">
                                    <tr>
                                        ...
                                        <th class="border p-2 text-left">{{ __( 'Vendor' ) }}</th>
                                        <th class="border p-2 text-left">{{ __( 'Products' ) }}</th>
                                        <th class="border p-2 text-left">{{ __( 'Unit Price' ) }}</th>
                                        ...
                                        <th width="150" class="border p-2">{{ __( 'SubTotal' ) }}</th>
                                        <th width="150" class="border p-2">{{ __( 'Commission' ) }}</th>
                                        <th width="150" class="border p-2">{{ __( 'Net' ) }}</th>
                                    </tr>
                                </thead>
                                <tbody class="text-primary">
                                    <tr v-for="product of result" :key="product.id">
                                        <td class="p-2 border">@{{ product.created_at }}</td>
                                        <td class="p-2 border text-right">@{{ product.vendor.vendor_id }}</td>
                                        <td class="p-2 border text-right">@{{ product.name }}</td>
                                        ...
                                        <td class="p-2 border text-right">@{{ (product.total_price - product.discount - product.tax_value) | currency }}</td>
                                        <td class="p-2 border text-right">@{{ ((product.total_price - product.discount - product.tax_value)* product.vendor.commission)/100 | currency }}</td>
                                        <td class="p-2 border text-right">@{{ (product.total_price - product.discount - product.tax_value) - ((product.total_price - product.discount - product.tax_value)* product.vendor.commission)/100 | currency }}</td>
                                    </tr>
                                </tbody>
                                <tfoot class="text-primary font-semibold">
                                    <tr>
                                        ...
                                        <td class="p-2 border text-primary"></td>
                                        <td class="p-2 border text-primary"></td>
                                        <td class="p-2 border text-right text-primary">@{{ computeTotal( result, 'unit_price' ) | currency }}</td>
                                        ...
                                        <td class="p-2 border text-right text-primary">@{{ computeTotal( result, 'total_price' ) -  computeTotal( result, 'tax_value' ) - computeTotal( result, 'discount' )  | currency }}</td>
                                        <td class="p-2 border text-right text-primary">@{{ computeTotal( result, 'commission' ) | currency }}</td>
                                        <td class="p-2 border text-right text-primary">@{{ (computeTotal( result, 'total_price' ) -  computeTotal( result, 'tax_value' ) - computeTotal( result, 'discount' )) - computeTotal( result, 'commission' ) | currency }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <div class="bg-box-background shadow rounded my-4" v-if="reportType.value === 'categories_summary'">
                     ...
                     <div class="flex w-full">
                        <div class="my-4 flex justify-between w-full">
                            <div class="text-secondary">
                                <ul>
                                    <li class="pb-1 border-b border-dashed">{{ __( 'Rent Details     '  ) }} @{{ 'Calculated rent to be applied on ' + endDateFormated + '( for ' + billingStartDate + ' - ' + billingEndDate + ')' }}   @{{ selectedUserMonthlyRent | currency}}</li>
                                    <li class="pb-1 border-b border-dashed">{{ __( 'You Earned' ) }} @{{(computeTotal( result, 'total_price' ) -  computeTotal( result, 'tax_value' ) - computeTotal( result, 'discount' )) - computeTotal( result, 'commission' ) - selectedUserMonthlyRent | currency }}</li>
                                    <li class="pb-1 border-b border-dashed">{{ sprintf( __( 'By : %s' ), Auth::user()->username ) }}</li>
                                    <li class="pb-1 border-b border-dashed">Shawn's Flea Market    285 Prince Royal Drive   Berea, KY  40403    (859) 985-9499</li>
                                </ul>
                            </div>

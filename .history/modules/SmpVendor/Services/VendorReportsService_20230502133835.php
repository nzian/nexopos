...
class VendorReportsService
{
    ...
    public function getProductsReports($start, $end, $vendor_id = null)
    {
        ...
        if (! empty($vendor_id)) {
            $request->with(['products' => function ($query) use ($vendor_id) {
                $query->with(['vendor' => function ($ven) {
                    $ven->select(['id', 'commission','monthly_rent','vendor_id']);
        ...
        if($vendor_id == null) {
            $vendor = Vendor::select(['id', 'commission','monthly_rent'])->get();
            $vendor = $vendor->map(function ($vend) {
                return [
                    'commission' => $vend->commission,
                    'monthly_rent' => $vend->monthly_rent,
                ];
            });
            return [
                'commission' => Currency::define($vendor->sum('commission'))->getRaw(),
                'monthly_rent' => Currency::define($vendor->sum('monthly_rent'))->getRaw(),
            ];
        } else {
            $vendor = Vendor::select(['id', 'commission','monthly_rent'])->find($vendor_id);
        }

        $products = $orders->map(fn ($order) => $order->products)->flatten();
        $summary = $this->getSaleSummeryFromProducts($products);
        $productsIds = $products->map(fn ($product) => $product->id)->unique(); // $product->product_id replaced with product->id mean ordered product id

        return [
            'vendor' => $vendor,
            'result' => $productsIds->map(function ($id) use ($products) {
                $product = $products->where('id', $id)->first(); //all informaiotn in the order product table no product in the system
                $product->commission = (($product->total_price-$product->discount-$product->tax_value)* $product->vendor?->commission)/100;

                $filtredProdcuts = $products->where('id', $id)->all();

                $summable = [ 'quantity', 'discount','commission', 'wholesale_tax_value', 'sale_tax_value', 'tax_value', 'total_price_without_tax', 'total_price', 'total_price_with_tax', 'total_purchase_price' ];
    ...
    public function getSaleSummeryFromProducts($products)
    {
        $allSales = $products->map(function ($order) {
            $vendor = Vendor::select(['id', 'commission','monthly_rent'])->find($order->vendor_id);
            return [
                'subtotal' => $order->total_price,
                'sales_discounts' => $order->discount,
                'sales_taxes' => $order->tax_value,
                'sales_commission' => ($order->total_price * $vendor?->commission)/100,
                'total' => $order->total_price - (($order->total_price * $vendor?->commission)/100),
            ];
        });
        ...
        return [
            ...
            'sales_commission' => Currency::define($allSales->sum('sales_commission'))->getRaw(),

<template>
    <div class="picker mb-2 w-full">
        <label v-if="field && field.label && field.label.length > 0" class="block leading-5 font-medium text-gray-500">{{ field.label }}</label>
        <div class="ns-button">
            <button @click="visible = !visible" :class="field && field.label && field.label.length > 0 ? 'mt-1 border border-input-edge' : ''" class="bg-white rounded cursor-pointer w-full p-1 flex items-center text-primary">
